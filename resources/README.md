...
- **`logo.svg`** - Source SVG logo (512×512 viewBox)
  - Minimal design with parameter table background
  - Rose gradient (#b76e79) with abstract keyboard representing MIDI-CI functionality

### Generated Platform Icons

#### macOS
- **`midicci.icns`** - macOS icon bundle
...
#### Windows
- **`midicci.ico`** - Windows icon file
  - Multi-resolution icon (16×16 to 256×256)
  - Embedded via `midicci.rc` resource script
  - Compiled into executable during build

#### Linux
- **`flatpak/dev.atsushieno.midicci.svg`** - Symlinked/copied from logo.svg
  - Installed to `${DATAROOTDIR}/icons/hicolor/scalable/apps/`
  - Referenced in `.desktop` file as `Icon=dev.atsushieno.midicci`
...
# Create macOS .icns
mkdir -p midicci.iconset
for size in 16 32 128 256 512; do
  cp icons/icon_${size}x${size}.png midicci.iconset/icon_${size}x${size}.png
done
for size in 32 64 256 512 1024; do
  half=$((size/2))
  cp icons/icon_${size}x${size}.png midicci.iconset/icon_${half}x${half}@2x.png
done
iconutil -c icns midicci.iconset -o midicci.icns
rm -rf midicci.iconset

# Create Windows .ico
magick icons/icon_{16x16,32x32,48x48,64x64,128x128,256x256}.png midicci.ico

# Update Linux icon
cp logo.svg ../flatpak/dev.atsushieno.midicci.svg
```
