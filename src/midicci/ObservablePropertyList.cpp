            ...
            common_rules_client->add_property_catalog_updated_callback([this]() {
                ...
                for (const auto& metadata : metadata_list) {
                    ...
                    if (existing_it != values_.end()) {
                        ...
                        auto* common_rules_metadata = dynamic_cast<CommonRulesPropertyMetadata*>(metadata.get());
                        std::string media_type = common_rules_metadata && !common_rules_metadata->mediaTypes.empty()
                            ? common_rules_metadata->mediaTypes[0]
                            : "application/json";
