...
public:
    ...
    MutableStateList<PropertyValue> properties_;
...
void ClientConnectionModel::unsubscribe_property(const std::string& resource, const std::string& res_id) {
    std::lock_guard<std::mutex> lock(pimpl_->mutex_);
    if (!pimpl_->connection_) return;
            ...
            if (pimpl_->connection_) {
                ...
                if (observable_properties) {
                    ...
                    auto it = std::find_if(properties_vec.begin(), properties_vec.end(),
                        [&propertyId](const PropertyValue& prop) {
